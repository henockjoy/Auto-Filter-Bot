...
async def safe_send_cached_media(client, **kwargs):
    while True:
        try:
            ...
            print(f"FloodWait detected: {e.value} seconds")
...
async def start(client, message):
    ...
    if (len(message.command) != 2) or (len(message.command) == 2 and message.command[1] == 'start'):
        buttons = [[
            InlineKeyboardButton("+ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ +", url=f'https://t.me/{temp.U_NAME}?startgroup=start', style=enums.ButtonStyle.PRIMARY)
        ],[
...
async def get_grp_stg(group_id):
    ...
    btn = [[InlineKeyboardButton('Edit IMDb template', callback_data=f'imdb_setgs#{group_id}')
    ],[
