...
from utils import get_plan_name, is_premium, get_size, is_subscribed, is_check_admin, get_wish, get_shortlink, get_readable_time, get_poster, temp, get_settings, save_group_settings
...
async def languages_(client: Client, query: CallbackQuery):
    ...
    if len(parts) < 4:
        key, req, offset = None, None, 0
        return

    _, key, req, raw_offset = parts
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.edit_message_media(
            InputMediaPhoto(random.choice(PICS), caption=script.STATUS_TXT.format(users, prm, chats, used_data_db_size, files, used_files_db_size, uptime)),
            reply_markup=InlineKeyboardMarkup(buttons)
        )
